...
class MujocoMulti(MultiAgentEnv):
    def __init__(self, batch_size=None, **kwargs):
        ...
        if self.enable_slip_detection:
            ...
            # 新增：交替步态检测参数
            self.alternation_reward_weight = env_args.get('alternation_reward_weight', 0.05)
            self.contact_height_threshold = env_args.get('contact_height_threshold', 0.04)
            self.last_contact_event = None  # "left" 或 "right" 或 None
            self.prev_contacts = {}  # 记录上一帧的接触状态
            
            print(f"滑动惩罚已启用 - 权重: {self.slip_penalty_weight}")
            print(f"交替步态奖励已启用 - 权重: {self.alternation_reward_weight}")
        else:
            print("滑动惩罚未启用")
        pass
    ...
    def step(self, actions):
        ...
        if hasattr(self, 'enable_slip_detection') and self.enable_slip_detection:
            ...
            # === 新增：计算交替步态奖励 ===
            alternation_bonus = self._calculate_alternation_reward()
            info['alternation_bonus'] = alternation_bonus
        ...
        # === 修改奖励计算：加入交替奖励 ===
        modified_reward = reward_n - slip_penalty + alternation_bonus
        rewards = [[modified_reward]] * self.n_agents
    ...
    def _calculate_slip_penalty_humanplus_style(self):
        ...
        try:
            ...
            force_slip_penalty = 0.0
            if contact_forces and len(contact_forces) > 0:
                force_slip_penalty = self._detect_force_based_slip(contact_forces) * 0.3
            
            # 综合惩罚
            total_penalty = (velocity_slip_penalty + force_slip_penalty) * self.slip_penalty_weight
            
            # 严格限制惩罚上限
            total_penalty = np.clip(total_penalty, 0, 0.1)
    ...
    def _detect_velocity_based_slip(self, current_positions):
        ...
        try:
            for i in range(min(len(current_positions), len(self.last_foot_positions))):
                # 计算水平速度
                horizontal_displacement = np.linalg.norm(
                    current_positions[i][:2] - self.last_foot_positions[i][:2]
                )
                horizontal_velocity = horizontal_displacement / dt
                
                # 足部高度和垂直速度
                foot_height = current_positions[i][2]
                vertical_velocity = 0
                if len(self.last_foot_positions) > i:
                    vertical_velocity = (current_positions[i][2] - self.last_foot_positions[i][2]) / dt
                
                # 只在以下情况下应用滑动惩罚：
                # 1. 足部接触地面（高度很低）
                # 2. 足部不在抬起状态（垂直速度不是向上）
                # 3. 水平速度确实过大
                is_on_ground = foot_height < 0.05
                is_not_lifting = vertical_velocity < 0.5
                is_excessive_slip = horizontal_velocity > self.max_slip_velocity
                
                # 只有同时满足这三个条件才给惩罚
                if is_on_ground and is_not_lifting and is_excessive_slip:
                    # 使用线性惩罚而不是平方
                    excess_velocity = horizontal_velocity - self.max_slip_velocity
                    total_penalty += excess_velocity
        except Exception as e:
            print(f"速度滑动检测错误: {e}")
        
        # 更新位置记录
        self.last_foot_positions = current_positions.copy()
        return total_penalty
    ...
    def _get_foot_contacts(self):
        ...
        try:
            for foot_idx in self.feet_indices:
                z = float(sim.data.body_xpos[foot_idx][2])
                contacts[foot_idx] = (z < self.contact_height_threshold)
        except Exception as e:
            print(f"获取足部接触状态错误: {e}")
            for foot_idx in self.feet_indices:
                contacts[foot_idx] = False
        
        return contacts

    def _calculate_alternation_reward(self):
        """计算交替步态奖励"""
        try:
            ...
            for foot_idx in self.feet_indices:
                try:
                    body_name = sim.model.body_id2name(foot_idx)
                    if body_name:
                        name_lower = body_name.lower()
                        if 'left' in name_lower or 'l_' in name_lower:
                            left_idx = foot_idx
                        elif 'right' in name_lower or 'r_' in name_lower:
                            right_idx = foot_idx
                except:
                    pass
            
            # 方法2：如果名称识别失败，使用索引顺序（假设第一个是左，第二个是右）
            if left_idx is None or right_idx is None:
                left_idx = self.feet_indices[0]
                right_idx = self.feet_indices[1]
            
            # 初始化上一帧接触状态
            if not hasattr(self, 'prev_contacts') or not self.prev_contacts:
                self.prev_contacts = {k: False for k in self.feet_indices}
            
            # 检测接触事件（从不接触到接触）
            for foot_idx in [left_idx, right_idx]:
                now_contact = contacts_now.get(foot_idx, False)
                prev_contact = self.prev_contacts.get(foot_idx, False)
                
                # 检测到新的接触事件（脚刚着地）
                if now_contact and not prev_contact:
                    # 判断是左脚还是右脚
                    side = 'left' if foot_idx == left_idx else 'right'
                    
                    # 检查是否与上一次接触事件交替
                    if hasattr(self, 'last_contact_event') and self.last_contact_event is not None:
                        if self.last_contact_event != side:
                            # 成功交替！给予奖励
                            alternation_bonus += self.alternation_reward_weight
                    
                    # 更新最近一次接触事件
                    self.last_contact_event = side
            
            # 更新接触状态记录
            self.prev_contacts = contacts_now.copy()
            
            return alternation_bonus
            
        except Exception as e:
            print(f"交替步态奖励计算错误: {e}")
            return 0.0    
